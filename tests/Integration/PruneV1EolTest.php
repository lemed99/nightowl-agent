...
class PruneV1EolTest extends TestCase
{
        ...
        if (self::$pdo !== null) {
            foreach (array_keys(\NightOwl\Support\StorageV2::TABLES) as $v1) {
                ...
                if ($missing && $backupExists) {
                    ...
                    // The shape backup carries every default except id's (see
                    // backupAllShapes); give the restored table a self-contained
                    // identity so later suites can insert into it.
    ...
    private function backupAllShapes(): void
    {
        foreach (array_keys(\NightOwl\Support\StorageV2::TABLES) as $v1) {
            ...
            // Take the defaults — a plain LIKE dropped every one of them, and a
            // table restored from that backup then rejected any insert omitting a
            // NOT NULL DEFAULT column (nightowl_mail.cc, 23502). That only bites
            // when the retire test runs BEFORE an insert test in this class, so it
            // stayed invisible in declaration order and surfaced the first time
            // phpunit.xml's executionOrder="defects" reshuffled the class.
            self::$pdo->exec("CREATE TABLE __{$v1}_shape_backup (LIKE {$v1} INCLUDING DEFAULTS)");
            // ...then drop the one default that must NOT be copied: id's nextval()
            // makes the backup depend on the source's id sequence, which would
            // block the very DROP under test with 2BP01. The restore re-adds an
            // identity in its place.
            self::$pdo->exec("ALTER TABLE __{$v1}_shape_backup ALTER COLUMN id DROP DEFAULT");
        }
    }
