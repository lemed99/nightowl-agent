...
src/Support/
  ...
  NightwatchIngestArguments.php — completes the provider's Ingest argument list against the INSTALLED nightwatch constructor: 1.29+ requires `Dispatcher $events` (IngestingEvents veto hook), 1.26–1.28 reject the name. Alone → the app dispatcher; parallel → an empty one (Nightwatch's own ingest already fires the hook; a shared dispatcher double-counts a limiter). NightwatchCompatibilityTest constructs the installed Ingest with the real list and names any required parameter we omit; CI runs a `--prefer-lowest` unit leg so both ends of `^1.26` stay tested while the lock tracks the newest.
  ...
  TableCatalog.php       — One statement answering "which of these tables exist, and what relkind" (to_regclass semantics, schema-relative). Replaces per-table hasTable/to_regclass probes on the deploy path — round trips, not rows, are what made nightowl:migrate slow on a distant DB
  RouteAction.php        — Collapses laravel/nightwatch's Livewire route actions to their SORTED DISTINCT components before the dict_route hash is taken. Nightwatch appends one class name per hydrated component with no dedupe (CapturesState:541), and a page whose components all poll or all react to one event sends them in one request — 78 identical cards = a 2,884-byte action. Deduped not run-length encoded (a count is the part that keeps changing, so it fixes size and not cardinality); SORTED because order-sensitivity alone mints a row per hydration rotation; NEVER truncated (a cap merged different pages, since on the shared livewire.update route the action is the only field telling them apart, and bounded length rather than cardinality). Does NOT collapse a page to one row — component MEMBERSHIP is part of the value, so conditional rendering still yields one row per distinct set
  PayloadCapture.php / CapturingIngest.php — NightOwl's own request-payload + response-body capture (`capture` config key). Nightwatch hard-codes payload capture to 500s (RequestSensor, final/@internal) and has no response bodies, so CapturingIngest wraps ONLY the NightOwl-bound ingest (never Nightwatch's own in parallel mode) and rewrites `t=request` records at write(). Inputs: the RequestRecord via Core::redactRequests() (runs in the same call that writes the record → exact pairing, and read at write time so customer redactRequests edits are honored) + the Response via RequestHandled; both cleared on use (Octane), method+status must agree. Filters (status codes/classes, IgnoredRoutes patterns) set-but-all-invalid → capture NOTHING. Hard ~4 MB wire ceiling → TOO_LARGE marker (AsyncServer refuses >10 MB frames = whole request lost). Fails open. Proven against the REAL Nightwatch in PayloadCaptureEndToEndTest
  IgnoredRoutes.php      — Glob matching for nightowl.ignore_routes (path/name/action, leading slash optional both sides)
...
src/Commands/
  ...
  RepairCacheRollupKeysCommand.php — nightowl:repair-cache-rollup-keys (re-aggregate legacy literal-key cache rollup rows onto their CacheKeyTemplate patterns; rebuild-and-swap by default, --in-place fallback)
...
| `nightowl:migrate` | Idempotent schema sync — `migrate --database=nightowl` (history in the nightowl DB) + baseline adoption of an already-present schema. Run on each deploy. **Auto-backfills** any rollup table it leaves existing-but-empty (the API read path serves zeros off an empty rollup, so migrate populates it from raw immediately), and reconciles rollup completeness on **both** bounds: a minute table missing raw history (min-vs-min, a base that started late) OR one whose newest bucket trails the newest raw row by more than `TAIL_FREEZE_TOLERANCE_SECONDS` (max-vs-max, a base that STOPPED) gets the full chain; a tier whose call_count sum falls short of its chain source (gaps, incl. mid-history holes) gets `nightowl:backfill-rollups --tiers-only` (minute→hour→day re-aggregation, no raw scan). The tail arm exists because a freeze is invisible to the other two — the floor never moves, and a frozen minute base freezes its hourly child too, so their sums keep agreeing (Yomoney concurrency-rollup freeze, 2026-07-31). Its tolerance is loose on purpose: bucket starts are event time and `created_at` is the drain-insert clock, so a drain backlog widens the gap with nothing wrong, while a real freeze grows without bound. It assumes every raw row reaches a bucket — true only because every RollupSpec groups on `COALESCE(...)`; a spec that FILTERED rows would need its own predicate. Also repairs whatever the drain marked in `nightowl_settings.rollup_repair_from` (a batch that couldn't take the shared rollup lock) and clears the marker. An empty base over an EMPTY raw source is complete (nothing to roll up), not a hole. **Round-trip budget is the constraint here, not row counts**: every measurement for every base rides ONE statement (`completenessSql` — bounds, raw floor/ceiling across both families, tier sums, under one snapshot) and table presence comes from one `TableCatalog` statement per phase; the whole steady-state command is ~16 statements. It was ~250, each a microsecond catalog lookup paying a full network round trip, and a customer with a ~100ms app→DB hop measured 72s per deploy on an idle dev box and on prod alike (tinybit.farm, 2026-08-25; `MigrateRoundTripTest` pins the count). Never add a per-table query to this path. Skip with `--no-backfill`. |
...
| `nightowl:repair-cache-rollup-keys` | Collapse literal cache keys in **existing** `nightowl_cache_*_rollups` rows onto their `CacheKeyTemplate` pattern, merging the counters. Templating only bounds cardinality for rows written AFTER the upgrade; rows already on disk keep near-unique keys, and because the tier backfill collapses ~0% of them one bloated minute table becomes three (field-measured: 45.6M minute → 45.3M hourly + 45.4M daily). Once those tables and their 4-column PKs no longer fit in cache the drain falls behind ingest and the agent starts rejecting payloads. Regroups only — no bucket, no counter, no chart point is lost. Idempotent; `--dry-run`/`--tier=`. **Default = rebuild and swap** (`CacheRollupSwap`, the pg_repack shape): build the collapsed copy beside the live table from a `pg_export_snapshot()` snapshot on a second connection while an AFTER INSERT/UPDATE/DELETE trigger records the drain's concurrent writes, fold the delta in, rename under the advisory lock in **ms**. Needs ~the largest table's size in free disk (`--force` skips that confirmation); leaves NO dead space, so no repack after; collapses the live bucket too. `--chunk-days=`. Deletes become NEGATIVE deltas — a tombstone would take a group's surviving keys with it. Pre-flights the snapshot handshake itself (two statements on a scratch transaction, BEFORE any trigger or delta table exists — a refused import used to cost a failed run per table; on failure it reports the server version and BOTH backend PIDs, since same-PID means the two connections collapsed onto one session and different-PID means something ran a query on the second one first — and in that case NAMES it, by switching the work connection's query log on for the length of the handshake, an empty log being the equally useful reading that the host app is not the cause; then offers `--in-place`, defaulting to NO, and under `--no-interaction` exits non-zero without starting), table ownership (the swap ends in DROP TABLE), and refuses a table carrying a column it does not know how to aggregate. The import is the FIRST statement in its transaction — isolation level rides on the `BEGIN`, both go through raw `PDO::exec` (simple protocol, no Laravel query events). **`--in-place`** = the old merge-in-the-live-table walk, for a disk-constrained tenant: batched by KEY (the PK's leading column) with an additive `ON CONFLICT`, paced by measured lock hold toward 1s, buckets inside the 10min drain safety margin skipped (so the current day's daily row stays literal — the command names the ceiling to come back to). Slower, leaves dead space until a `pg_repack`. `--since`/`--until`/`--key-batch`/`--sleep-ms` (the bounds are refused under the rebuild, which has no window) |
...
| `nightowl:gc-dict-routes` | One-off reclamation of `nightowl_dict_route` rows no `nightowl_requests_v2` row references. **Agent must be stopped everywhere it runs** — a drain worker caches route ids for its process lifetime and COPYs the referencing row later, so no in-band check can protect a cached id; a stopped agent has no cache and that is the whole protection. Probes: health port, drain metrics file, ingest buffer mtime (600s window — a normal shutdown leaves fresh files, so it refuses for up to 10 min after stopping and SAYS how long); all blind to `--driver=sync` and to other hosts (the confirmation says so). `--force` skips the confirmation, never the probes. In-band narrowing: `LOCK TABLE nightowl_requests_v2 IN SHARE MODE` for the whole run (a writer makes it wait; not granted within 30s → refuse; arriving writers stall rather than race). One transaction (pooler affinity + all-or-nothing), temp table `ON COMMIT DROP`, delete pages over the temp table's own ids (statement count ∝ rows, not id span). Dry run runs the probes and WARNS instead of refusing (it holds share locks during its scan). Advises `VACUUM FULL`/pg_repack — orphans interleave with live rows so plain VACUUM does not shrink the file. Exit 3 = operator declined. Not on a timer: a live GC would need dict_trace's clock + per-batch touch + no-LRU-trust, 2-4 round trips per batch forever. `--dry-run`/`--chunk` (≤50k)/`--sqlite-path` |
...
- **Drain write-path economics**: rollup upserts are multi-row (500 rows/statement, `ROLLUP_UPSERT_CHUNK`) — a 200-group batch is ~3 statements, not ~600; 22 audited-dead raw indexes dropped (000056 — every drop has a documented no-reader verdict; the string `timestamp` indexes, redundant single-column prefixes of the 000044 composites, and the duration singles); rollup tables run fillfactor 70 (000053) + autovacuum scale factor 0.02 (000055), with an ABSOLUTE 50k vacuum/analyze floor on the coarse tiers (000071) — 0.02 of a 574-row daily table is ~61 dead tuples, so the tier tables were being re-read from disk every `autovacuum_naptime` (a live tenant showed 3,227 lifetime vacuums on `nightowl_job_daily_rollups` vs 32 on the 1.3M-row `nightowl_query_rollups`, ~45 GB/hr of reads competing with the drain). 000071 DISCOVERS tables from the catalog rather than hand-listing, because the hand-lists in 000053/000054/000055 are why `nightowl_request_concurrency_rollups` (added later, 000063) and the CacheRollupSwap-rebuilt cache rollups sat in production with no reloptions at all. Drain benchmark: 16.7k rows/s with sketch dual-write (23.7k without), vs the 5.6k/s target.
...
- PII redaction is delegated upstream to `laravel/nightwatch` via `NIGHTWATCH_REDACT_HEADERS` / `NIGHTWATCH_REDACT_PAYLOAD_FIELDS`. The agent daemon itself does not redact — payloads are buffered and drained as received. The one exception runs in the customer's process, not the daemon: `PayloadCapture` redacts captured JSON response bodies with `NIGHTOWL_REDACT_RESPONSE_FIELDS` (and re-applies Nightwatch's payload list to payloads it captures).
...
NIGHTOWL_IGNORE_ROUTES=                  # Comma-separated patterns (`*` only wildcard, spans `/`; `?` is literal; leading slash optional). A matching request is dropped at drain time with the records of ITS OWN EXECUTION (queries/logs/cache/mail/notifications/outgoing whose execution_id is the request's). Scoped by execution_id, NOT trace_id: nightwatch propagates the trace into dispatched jobs, so trace-scoping dropped a job's own queries/mail only when co-batched with the request — batching decided a job's detail page. NEVER dropped: exception (kept deliberately; its "view request" link then leads nowhere, deterministically), queued-job (its execution_id IS the request's — dropping it while the later attempt survives = orphan attempt, QUEUED 0), job-attempt. Livewire 3 = `livewire/update` / `default.livewire.update`; Livewire 4 = `livewire-<APP_KEY hash>/update` / `default-livewire.update` — `*livewire.update` matches both. A drop, not a sample: absent from request count, thresholds AND error rate, in whichever direction the ignored route's own health sits (healthy route ignored → Err% rises; failing route ignored → Err% FALLS and hides it) — hence `app_vitals.requests_ignored`, shown beside Err%. Patterns validated at parse (non-string / invalid UTF-8 dropped with a log line: an invalid-UTF-8 pattern would make preg_match warn per record, which the daemon promotes to an exception that wedges the drain). Top-level key (shallow-merge rule)
NIGHTOWL_CAPTURE_REQUEST_PAYLOAD=         # Unset = Nightwatch's behaviour (payload on 500s only). true = every request (narrowed by the two filters below), false = never. Top-level key `capture` (shallow-merge rule)
NIGHTOWL_CAPTURE_RESPONSE_BODY=false     # Capture response bodies → nightowl_requests_v2.response_z (000075, v2 only; drain gates on the column)
NIGHTOWL_CAPTURE_STATUS_CODE=            # "422,5xx" — narrows BOTH captures. Empty = every status; set but all-invalid = nothing
NIGHTOWL_CAPTURE_ROUTES=                 # IgnoredRoutes patterns (path/name/action) — narrows BOTH. Empty = every route; set but all-invalid = nothing
NIGHTOWL_CAPTURE_MAX_BYTES=0             # Per-body truncation, 0 = no limit (the ~4 MB combined wire ceiling still applies)
NIGHTOWL_REDACT_RESPONSE_FIELDS=         # JSON response keys, any depth, case-insensitive. Unset/absent = PayloadCapture::DEFAULT_REDACT_RESPONSE_FIELDS; "" = off
