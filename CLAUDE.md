...
|-------|-------|--------------|-------|
| **Unit** | 387 | None | PayloadParser, MetricsCollector, ConnectionHandler, AlertNotifier, DrainWorker |
| **Integration** | 302 | SQLite always, PG skips if unavailable | SqliteBuffer (multi-worker claiming, WAL), RecordWriter (COPY/upsert/users_count), SimulatorPayload, EndToEnd |
| **System** | 47 | PG + pcntl + posix | Real AsyncServer + fork + drain over TCP; thresholds, back-pressure, multi-worker, error storms, scaling |

**Total**: 736 test methods.
